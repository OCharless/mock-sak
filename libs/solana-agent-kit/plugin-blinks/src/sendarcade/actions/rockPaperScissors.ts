        ...
        [
            {
                ...
                explanation: "Play Rock Paper Scissors betting 0.1 SOL and choosing rock",
            },
            {
                input: {
                    amount: 0.05,
                ...
                explanation: "Play Rock Paper Scissors betting 0.05 SOL and choosing paper",
            },
        ],
    ],
    schema: z.object({
        amount: z.number().positive().describe("Amount of SOL to bet"),
        choice: z.enum(["rock", "paper", "scissors"]).describe("Your choice for the game"),
    }),
    handler: async (agent, input) => {
        try {
            const result = await rock_paper_scissor(agent, input.amount, input.choice);
