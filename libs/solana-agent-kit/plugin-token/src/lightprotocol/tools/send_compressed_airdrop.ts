...
import { buildTx, calculateComputeUnitPrice } from "@lightprotocol/stateless.js";
import {
...
import { SolanaAgentKit, signOrSendTX } from "../../../../core/src";
...
export async function sendCompressedAirdrop(
    ...
    try {
        ...
        const associatedToken = getAssociatedTokenAddressSync(mintAddress, agent.wallet.publicKey);
...
async function processAll(
    ...
    const sourceTokenAccount = getAssociatedTokenAddressSync(mint, agent.wallet.publicKey);
    ...
    const tx = buildTx(transaction.instructions, agent.wallet.publicKey, blockhash, [
        lookupTableAccount,
    ]);
