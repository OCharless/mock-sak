...
import { type PublicKey, Transaction, type TransactionInstruction } from "@solana/web3.js";
...
export async function closeEmptyTokenAccounts(agent: SolanaAgentKit) {
    try {
        ...
        const token_2022 = await create_close_instruction(agent, TOKEN_2022_PROGRAM_ID);
        ...
        if (agent.config.signOnly) {
            return {
                signedTransaction: (await signOrSendTX(agent, transaction)) as Transaction,
                size,
            };
        }
