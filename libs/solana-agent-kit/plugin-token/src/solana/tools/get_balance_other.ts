import { LAMPORTS_PER_SOL, type ParsedAccountData, type PublicKey } from "@solana/web3.js";
...
export async function get_balance_other(
    ...
    try {
        if (!token_address) {
            return (await agent.connection.getBalance(wallet_address)) / LAMPORTS_PER_SOL;
        }
