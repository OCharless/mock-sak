...
export async function cancelLimitOrders(agent: SolanaAgentKit, params: CancelJupiterOrderRequest) {
    params.maker = agent.wallet.publicKey.toString();
    try {
        const data = await cancelOrdersApi(params);
        const transactions = data.txs.map((tx: string) => deserializeTransaction(tx));
