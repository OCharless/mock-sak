...
async function findTokenContract(symbol: string, chain: string): Promise<string> {
...
async function swapSolana(quote: Quote, agent: SolanaAgentKit, dstAddr: string): Promise<string> {
...
async function swapEVM(quote: Quote, agent: SolanaAgentKit, dstAddr: string): Promise<string> {
    if (!evmWallet) {
    ...
    const tokenContract = new Contract(quote.fromToken.contract, ERC20Permit_ABI.abi, signer);
