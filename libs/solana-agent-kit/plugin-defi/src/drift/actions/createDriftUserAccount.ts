...
const createDriftUserAccountAction: Action = {
    ...
    similes: ["create drift account", "create drift user account", "create user account on drift"],
    ...
    handler: async (agent, input) => {
        try {
            const res = await createDriftUserAccount(agent, input.amount, input.symbol);
