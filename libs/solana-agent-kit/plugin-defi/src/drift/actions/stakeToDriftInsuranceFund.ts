...
const stakeToDriftInsuranceFundAction: Action = {
    ...
    schema: z.object({
        amount: z.number().positive().describe("Amount to stake in normal units e.g 50 === 50 SOL"),
        symbol: z.string().describe("Symbol of the token stake"),
    }),
    handler: async (agent, input) => {
        try {
            const tx = await stakeToDriftInsuranceFund(agent, input.amount, input.symbol);
