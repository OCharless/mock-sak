...
async function getVaultAvailableBalance(agent: SolanaAgentKit, vault: string) {
    try {
        ...
        const currentVaultBalance = convertToNumber(vaultDetails.netDeposits, QUOTE_PRECISION);
...
export async function createVault(
    ...
    try {
        ...
        const tx = await vaultClient.initializeVault({
            ...
            managementFee: new BN(params.managementFee).mul(PERCENTAGE_PRECISION).div(new BN(100)),
...
export async function updateVault(
    ...
    try {
        ...
        const tx = await vaultClient.managerUpdateVault(vaultPublicKey, {
            redeemPeriod: params.redeemPeriod ? new BN(params.redeemPeriod * 86400) : null,
            maxTokens: params.maxTokens ? numberToSafeBN(params.maxTokens, spotPrecision) : null,
            ...
            managementFee: params.managementFee
                ? new BN(params.managementFee).mul(PERCENTAGE_PRECISION).div(new BN(100))
                : null,
            profitShare: params.profitShare
                ? new BN(params.profitShare).mul(PERCENTAGE_PRECISION).div(new BN(100)).toNumber()
                : null,
            hurdleRate: params.hurdleRate
                ? new BN(params.hurdleRate).mul(PERCENTAGE_PRECISION).div(new BN(100)).toNumber()
                : null,
...
export const deriveDriftVaultAddress = async (agent: SolanaAgentKit, name: string) => {
 ...
 * @returns
 */
export async function getVaultInfo(agent: SolanaAgentKit, vaultNameOrAddress: string) {
    try {
        const { vaultClient, cleanUp } = await initClients(agent);
        ...
        const data = {
            ...
            minDepositAmount: vaultDetails.minDepositAmount.div(spotToken.precision).toNumber(),
            managementFee:
                (vaultDetails.managementFee.toNumber() / PERCENTAGE_PRECISION.toNumber()) * 100,
            profitShare: (vaultDetails.profitShare / PERCENTAGE_PRECISION.toNumber()) * 100,
            hurdleRate: (vaultDetails.hurdleRate / PERCENTAGE_PRECISION.toNumber()) * 100,
...
*/
export async function depositIntoVault(agent: SolanaAgentKit, amount: number, vault: string) {
    const { vaultClient, driftClient, cleanUp } = await initClients(agent);
...
*/
export async function withdrawFromDriftVault(agent: SolanaAgentKit, vault: string) {
    try {
        const { vaultClient, cleanUp } = await initClients(agent);
        const vaultPublicKey = new PublicKey(vault);
        const isOwned = await getIsOwned(agent, vault);
...
export async function getVaultAddress(agent: SolanaAgentKit, name: string) {
    ...
    try {
        const { vaultClient, cleanUp } = await initClients(agent);
        const vaultAddress = getVaultAddressSync(vaultClient.program.programId, encodedName);
