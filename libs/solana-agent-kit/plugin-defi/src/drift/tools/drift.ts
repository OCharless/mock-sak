...
import type { SolanaAgentKit } from "../../../../core/src";
...
export async function initClients(
    ...
    const vaultProgram = new anchor.Program(IDL, VAULT_PROGRAM_ID, driftClient.provider);
...
export async function createDriftUserAccount(
    ...
    try {
        ...
        const token = MainnetSpotMarkets.find((v) => v.symbol === symbol.toUpperCase());
...
export async function depositToDriftUserAccount(
    ...
    try {
        ...
        const txSignature = await driftClient.txSender.sendRawTransaction(signedTx.serialize(), {
            ...driftClient.opts,
        });
        ...
        if (params.type === "limit") {
            ...
            signature = await driftClient.placePerpOrder(
                getLimitOrderParams({
                    ...
                    reduceOnly: false,
                    direction:
                        params.action === "long" ? PositionDirection.LONG : PositionDirection.SHORT,
...
export async function driftUserAccountInfo(agent: SolanaAgentKit) {
    try {
        ...
        const perpPositions = account.perpPositions.map((pos) => ({
            ...
            quoteAssetAmount: convertToNumber(pos.quoteAssetAmount.abs(), QUOTE_PRECISION),
            quoteEntryAmount: convertToNumber(pos.quoteEntryAmount.abs(), QUOTE_PRECISION),
            quoteBreakEvenAmount: convertToNumber(pos.quoteBreakEvenAmount.abs(), QUOTE_PRECISION),
            ...
            positionType:
                convertToNumber(pos.baseAssetAmount, BASE_PRECISION) > 0 ? "long" : "short",
        }));
        const spotPositions = account.spotPositions.map((pos) => {
            ...
            return {
                availableBalance:
                    ...
                    convertToNumber(tokenBalance, MainnetSpotMarkets[pos.marketIndex].precision),
        ...
        const netUSDValue = convertToNumber(overallUserBalance.add(unrealizedPnl), QUOTE_PRECISION);
...
export async function stakeToDriftInsuranceFund(
    ...
    try {
        ...
        const deriveInsuranceFundStakeAccount = getInsuranceFundStakeAccountPublicKey(
 ...
 * @param symbol
 */
export async function unstakeFromDriftInsuranceFund(agent: SolanaAgentKit, symbol: string) {
    try {
        const { driftClient, cleanUp } = await initClients(agent);
        const token = MainnetSpotMarkets.find((v) => v.symbol === symbol.toUpperCase());
...
export async function getEntryQuoteOfPerpTrade(
    ...
    try {
        ...
        return {
            entryPrice: convertToNumber(estimatedEntryPriceData.entryPrice, QUOTE_PRECISION),
            priceImpact: convertToNumber(estimatedEntryPriceData.priceImpact, QUOTE_PRECISION),
            bestPrice: convertToNumber(estimatedEntryPriceData.bestPrice, QUOTE_PRECISION),
            worstPrice: convertToNumber(estimatedEntryPriceData.worstPrice, QUOTE_PRECISION),
        };
    } catch (e) {
        // @ts-expect-error - error message is a string
 ...
 * @param symbol
 */
export async function getLendingAndBorrowAPY(agent: SolanaAgentKit, symbol: string) {
    try {
        const { driftClient, cleanUp } = await initClients(agent);
        const token = MainnetSpotMarkets.find((v) => v.symbol === symbol.toUpperCase());
