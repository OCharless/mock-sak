...
const createMeteoraDLMMPoolAction: Action = {
    ...
    schema: z.object({
        ...
        initialPrice: z.number().positive().describe("Initial pool price in ratio tokenA / tokenB"),
        priceRoundingUp: z
            .boolean()
            .describe("Whether to rounding up the initial pool price")
            .optional(),
        feeBps: z.number().positive().describe("Pool trading fee in BPS"),
        activationType: z.enum(["Timestamp", "Slot"]).describe("Pool activation type").optional(),
        hasAlphaVault: z
            .boolean()
