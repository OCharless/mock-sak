...
const createMeteoraDynamicAMMPoolAction: Action = {
    ...
    schema: z.object({
        ...
        tokenAAmount: z.number().positive().describe("The token A amount in lamport units"),
        tokenBAmount: z.number().positive().describe("The token B amount in lamport units"),
        tradeFeeNumerator: z.number().positive().describe("Trade fee numerator"),
        activationType: z.enum(["Timestamp", "Slot"]).describe("Activation type").optional(),
    ...
    handler: async (agent, input) => {
        try {
            ...
            const tradeFeeNumerator = new BN(input.tradeFeeNumerator.toString()).toNumber();
            const activationType = input.activationType ?? 1;
            const activationPoint = input.activationPoint ? new BN(input.activationPoint) : null;
