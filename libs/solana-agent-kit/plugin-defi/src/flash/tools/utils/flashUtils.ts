...
import { PerpetualsClient, PoolConfig, Privilege, Referral, Token } from "flash-sdk";
    ...
    if (nftReferralAccountInfo) {
        ...
        if (nftTradingAccountPk) {
            const nftTradingAccountInfo =
                await perpClient.provider.connection.getAccountInfo(nftTradingAccountPk);
            if (nftTradingAccountInfo) {
                ...
                const accountExists = await perpClient.provider.connection.getAccountInfo(
