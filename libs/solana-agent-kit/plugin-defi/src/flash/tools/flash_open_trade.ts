...
export async function flashOpenTrade(
    ...
    try {
        ...
        const collateralToken = ALL_TOKENS.find((t) => t.symbol === collateralSymbol);
...
function calculateCollateralAmount(usdAmount: number, tokenPrice: number, decimals: number): BN {
...
async function fetchCustodyAccounts(
    ...
    return {
        targetCustody: CustodyAccount.from(
            targetConfig.custodyAccount,
            perpClient.program.coder.accounts.decode<Custody>("custody", accounts[0].data)
        ),
        collateralCustody: CustodyAccount.from(
            collateralConfig.custodyAccount,
            perpClient.program.coder.accounts.decode<Custody>("custody", accounts[1].data)
        ),
    };
}
