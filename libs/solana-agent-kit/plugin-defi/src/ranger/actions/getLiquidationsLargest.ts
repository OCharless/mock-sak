...
export type GetLiquidationsLargestInput = z.infer<typeof getLiquidationsLargestSchema>;
...
export const getLiquidationsLargestAction: Action = {
    ...
    similes: ["get largest liquidations", "fetch largest liquidations", "liquidations biggest"],
    ...
    handler: async (
        ...
        if (!response.ok) {
            ...
            throw new Error(`Get liquidations largest request failed: ${error.message}`);
        }
