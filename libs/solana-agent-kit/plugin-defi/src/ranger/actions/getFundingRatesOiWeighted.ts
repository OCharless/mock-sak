...
export type GetFundingRatesOiWeightedInput = z.infer<typeof getFundingRatesOiWeightedSchema>;
...
export const getFundingRatesOiWeightedAction: Action = {
    ...
    handler: async (
        ...
        const response = await fetch(`${RANGER_DATA_API_BASE}/v1/funding_rates/oi_weighted`, {
            method: "GET",
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Get funding rates oi weighted request failed: ${error.message}`);
        }
