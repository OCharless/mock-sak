...
export type GetLiquidationsCapitulationInput = z.infer<typeof getLiquidationsCapitulationSchema>;
...
export const getLiquidationsCapitulationAction: Action = {
    ...
    handler: async (
        ...
        if (input.threshold !== undefined) params.set("threshold", input.threshold.toString());
        ...
        if (!response.ok) {
            ...
            throw new Error(`Get liquidations capitulation request failed: ${error.message}`);
        }
