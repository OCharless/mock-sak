...
export type GetFundingRatesAccumulatedInput = z.infer<typeof getFundingRatesAccumulatedSchema>;
...
export const getFundingRatesAccumulatedAction: Action = {
    ...
    similes: ["get funding rates accumulated", "fetch accumulated funding", "funding rates sum"],
    ...
    handler: async (
        ...
        if (!response.ok) {
            ...
            throw new Error(`Get funding rates accumulated request failed: ${error.message}`);
        }
