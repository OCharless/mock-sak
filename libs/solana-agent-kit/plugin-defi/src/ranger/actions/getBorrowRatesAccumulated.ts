...
export type GetBorrowRatesAccumulatedInput = z.infer<typeof getBorrowRatesAccumulatedSchema>;
...
export const getBorrowRatesAccumulatedAction: Action = {
    ...
    similes: ["get borrow rates accumulated", "fetch accumulated borrow rates", "borrow rates sum"],
    ...
    handler: async (
        ...
        if (!response.ok) {
            ...
            throw new Error(`Get borrow rates accumulated request failed: ${error.message}`);
        }
