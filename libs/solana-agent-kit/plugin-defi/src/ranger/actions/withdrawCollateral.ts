...
export const withdrawCollateralAction: Action = {
    ...
    handler: async (
        ...
        const response = await fetch(`${RANGER_SOR_API_BASE}/v1/withdraw_collateral`, {
            method: "POST",
            headers: {
        ...
        const signature = await agent.wallet.signAndSendTransaction(transaction, agent.connection);
