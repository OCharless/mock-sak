...
export type GetLiquidationsHeatmapInput = z.infer<typeof getLiquidationsHeatmapSchema>;
...
export const getLiquidationsHeatmapAction: Action = {
    ...
    similes: ["get liquidations heatmap", "fetch liquidation heatmap", "liquidations by time"],
    ...
    handler: async (
        ...
        if (!response.ok) {
            ...
            throw new Error(`Get liquidations heatmap request failed: ${error.message}`);
        }
