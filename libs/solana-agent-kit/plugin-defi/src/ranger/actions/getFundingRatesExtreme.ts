...
export type GetFundingRatesExtremeInput = z.infer<typeof getFundingRatesExtremeSchema>;
...
export const getFundingRatesExtremeAction: Action = {
    ...
    similes: ["get funding rates extreme", "fetch extreme funding", "funding rates outliers"],
    ...
    handler: async (
        ...
        if (!response.ok) {
            ...
            throw new Error(`Get funding rates extreme request failed: ${error.message}`);
        }
