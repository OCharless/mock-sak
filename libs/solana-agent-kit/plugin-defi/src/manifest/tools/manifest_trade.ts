...
export async function limitOrder(
    ...
    try {
        ...
        const depositPlaceOrderIx: TransactionInstruction[] =
            await mfxClient.placeOrderWithRequiredDepositIxs(agent.wallet.publicKey, orderParams);
 ...
 * @returns Transaction signature
 */
export async function cancelAllOrders(agent: SolanaAgentKit, marketId: PublicKey) {
    try {
        const mfxClient = await ManifestClient.getClientForMarketNoPrivateKey(
            agent.connection,
            marketId,
...
export async function batchOrder(
    ...
    try {
        ...
        const placeParams: WrapperPlaceOrderParamsExternal[] = orders.map((order) => ({
        ...
        const batchOrderIx: TransactionInstruction = mfxClient.batchUpdateIx(placeParams, [], true);
