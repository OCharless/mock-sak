...
import { signOrSendTX } from "../../../../core/src";
...
function applySlippage(nb: BN, percentage: number): BN {
    ...
    const percentageBN = new BN((negative ? percentage * -1 : percentage) * 10_000);
...
export async function closePerpTradeShort({
    ...
    const stakingRewardTokenCustodyTokenAccount = AdrenaClient.findCustodyTokenAccountAddress(
        AdrenaClient.stakingRewardTokenMint
    );

    const position = AdrenaClient.findPositionAddress(owner, custody.pubkey, "long");
    ...
    const receivingAccount = AdrenaClient.findATAAddressSync(owner, collateralCustody.mint);

    const preInstructions: TransactionInstruction[] = [];

    const collateralCustodyOracle = collateralCustody.oracle;
    const collateralCustodyTokenAccount = AdrenaClient.findCustodyTokenAccountAddress(
        collateralCustody.mint
    );

    if (!(await AdrenaClient.isAccountInitialized(agent.connection, receivingAccount))) {
...
export async function closePerpTradeLong({
    ...
    const custodyTokenAccount = AdrenaClient.findCustodyTokenAccountAddress(tradeMint);
...
export async function openPerpTradeLong({
    ...
    const principalCustodyTokenAccount = AdrenaClient.findCustodyTokenAccountAddress(tradeMint);
    ...
    const priceWithSlippage = applySlippage(new BN(price * 10 ** PRICE_DECIMALS), slippage);
    ...
    if (!(await AdrenaClient.isAccountInitialized(agent.connection, collateralAccount))) {
        preInstructions.push(
            AdrenaClient.createATAInstruction({
...
export async function openPerpTradeShort({
    ...
    const collateralCustody = AdrenaClient.findCustodyAddress(instructionCollateralMint);
