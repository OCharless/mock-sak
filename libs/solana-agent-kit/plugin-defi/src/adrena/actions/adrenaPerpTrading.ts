...
const openPerpTradeLongAction: Action = {
        ...
        [
            {
                ...
                output: {
                    status: "success",
                    signature: "2ZE7Rz...",
                    message: "Successfully opened 2x long SOL position with 1000 USDC collateral",
                },
                explanation:
    ...
    schema: z.object({
        ...
        collateralAmount: z.number().positive().describe("Amount of collateral to use"),
        leverage: z
            .number()
            .positive()
            .default(2000)
        ...
        slippage: z.number().positive().default(0.3).describe("Slippage tolerance in percentage"),
    }),
...
const openPerpTradeShortAction: Action = {
    ...
    description: "Open a short perpetual trading position on Adrena with leverage",
    examples: [
        [
            {
                input: {
                ...
                output: {
                    status: "success",
                    signature: "2ZE7Rz...",
                    message: "Successfully opened 2x short SOL position with 1000 USDC collateral",
                },
                explanation:
...
const closePerpTradeLongAction: Action = {
    ...
    schema: z.object({
        price: z.number().positive().describe("Current price of the asset"),
        tradeMint: z.string().describe("Mint address of the token position to close"),
    }),
    handler: async (agent, input) => {
        try {
