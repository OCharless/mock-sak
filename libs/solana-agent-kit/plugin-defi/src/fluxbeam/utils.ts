...
export async function getTokenDecimals(agent: SolanaAgentKit, mint: PublicKey): Promise<number> {
    try {
        return (await getMint(agent.connection, mint, "finalized", TOKEN_PROGRAM_ID)).decimals;
    } catch (_error) {
        try {
            return (await getMint(agent.connection, mint, "finalized", TOKEN_2022_PROGRAM_ID))
                .decimals;
