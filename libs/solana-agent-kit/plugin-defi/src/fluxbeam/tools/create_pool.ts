...
export async function fluxBeamCreatePool(
    ...
    try {
        ...
        const tokenA_Decimals = isTokenA_NativeSol ? 9 : await getTokenDecimals(agent, token_a);

        const scaledAmountTokenA = token_a_amount * Math.pow(10, tokenA_Decimals);
        const isTokenB_NativeSol = token_b.equals(TOKENS.SOL);
        const tokenB_Decimals = isTokenB_NativeSol ? 9 : await getTokenDecimals(agent, token_b);
