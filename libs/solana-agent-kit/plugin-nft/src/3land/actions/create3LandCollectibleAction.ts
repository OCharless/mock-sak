import type { Action } from "../../../../core/src";
import { z } from "zod";
import { createCollection, createSingle } from "../tools/create_3land_collectible";
        ...
        [
            {
                ...
                explanation: "Create and list a single NFT on 3Land devnet with 5% royalties",
            },
        ],
    ],
    schema: z.object({
        isCollection: z.boolean().describe("Whether to create a collection or single NFT"),
    ...
    handler: async (_agent, input: Record<string, any>) => {
        try {
            ...
            if (!input.collectionAccount) {
                throw new Error("Collection account is required for single NFT creation");
            }
