import { Action, SolanaAgentKit } from "../../../../core/src";
import { z } from "zod";
import {
...
export const elfaGetSmartMentionsAction: Action = {
    ...
    schema: z.object({
        limit: z.number().describe("Number of tweets to retrieve (default: 100)").optional(),
...
export const elfaGetTopMentionsByTickerAction: Action = {
    ...
    schema: z.object({
        ...
        timeWindow: z.string().min(1).optional().describe("Time window for mentions (default: 1h)"),
...
export const elfaSearchMentionsByKeywordsAction: Action = {
    ...
    similes: ["search mentions", "find mentions by keywords", "tweets by keywords"],
    description:
    ...
    schema: z.object({
        keywords: z.string().min(1).describe("Keywords to search for string separated by commas"),
        from: z.number().describe("Start date as unix timestamp"),
        to: z.number().describe("End date as unix timestamp"),
        limit: z.number().optional().describe("Number of tweets to retrieve (default: 20)"),
    }),
    handler: async (agent: SolanaAgentKit, input) => {
        ...
        const data = await searchMentionsByKeywords(agent, keywords, from, to, limit);
        return {
            status: "success",
            data,
...
export const elfaSmartTwitterAccountStats: Action = {
    ...
    schema: z.object({
        username: z.string().min(1).describe("Twitter username to retrieve stats for"),
    }),
    handler: async (agent: SolanaAgentKit, input) => {
