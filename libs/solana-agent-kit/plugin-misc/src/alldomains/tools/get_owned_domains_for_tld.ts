 ...
 */
export async function getOwnedDomainsForTLD(agent: SolanaAgentKit, tld: string): Promise<string[]> {
    try {
        const domains = await new TldParser(agent.connection).getParsedAllUserDomainsFromTld(
