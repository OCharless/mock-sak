...
import { PriorityFeeResponse, SolanaAgentKit } from "../../../../core/src";
...
export async function sendTransactionWithPriorityFee(
    ...
    try {
        if (!splmintAddress) {
            ...
            const txSignature = await agent.wallet.signAndSendTransaction(transaction);
            ...
            const fromAta = await getAssociatedTokenAddress(splmintAddress, agent.wallet.publicKey);
