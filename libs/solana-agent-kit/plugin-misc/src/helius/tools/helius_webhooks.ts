...
import { HeliusWebhookIdResponse, HeliusWebhookResponse } from "../../../../core/src";
...
export async function getHeliusWebhook(
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `Failed to fetch webhook with ID ${webhookID}. ` + `Status Code: ${response.status}`
            );
        }
 ...
 */
export async function deleteHeliusWebhook(agent: SolanaAgentKit, webhookID: string): Promise<any> {
    try {
        const apiKey = agent.config?.HELIUS_API_KEY;
        if (!apiKey) {
