 ...
 */
export async function getPrimaryDomain(agent: SolanaAgentKit, account: PublicKey): Promise<string> {
    try {
        const { reverse, stale } = await _getPrimaryDomain(agent.connection, account);
        if (stale) {
